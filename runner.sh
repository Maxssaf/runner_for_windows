...
do	
	...
	cd ~/runner_for_windows
   	...
   	then	
		clear
		echo -e "[\033[1;32m$(date +"%d-%m-%Y %T")\033[1;0m] - Running up to date maxssaf"
	else
		cd ~/runner_for_windows
		clear
	...
	then	
		...
		then
			random_numbers=$(shuf -i 1-$list_size -n1)
		else
			random_numbers=$(shuf -i 1-$list_size -n1)
		fi
	...
	then
   	...
   	do
            ...
            echo "python runner.py $cmd_line $proxy_interval --rpc $rpc -t $threads --table"
            
            cd ~/mhddos_proxy
   
            python runner.py $cmd_line $proxy_interval --rpc $rpc -t $threads --table&
